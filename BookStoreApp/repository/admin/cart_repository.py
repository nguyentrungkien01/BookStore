...
def get_book_in_cart(**kwargs):
    query = db.session.query(CartModel.cart_id, BookModel.name, CategoryModel.name, BookModel.image,cart_detail_model.c.amount,\
...
def get_total_by_cartId(cart_id = 0, **kwargs):
    query = db.session.query(BookModel.price, cart_detail_model.c.amount).filter(cart_detail_model.c.book_id == BookModel.book_id)\
