...
function setCartData(cart, book){
    ...
    for(let i =0; i < cart.length; i++){
        ...
        var child =     `<div id="" class="info-user">
                            <div class="name-user">${cartId}. Tên tài khoản: ${cart[i]['username']}</div>
                            <div class="first-name">Họ tên: ${cart[i]['last_name']} ${cart[i]['first_name']}</div>
                            <div class="number-phone">Số điện thoại: ${cart[i]['phone_number']}</div>
                            <div class="total-money">Tổng tiền: ${format(cart[i]['total_money'])}</div>
                         </div>`;
        ...
        for(let j = 0; j <book.length; j++){
            if(cart[i]['cart_id'] == book[j]['cart_id']){
                bookx += `<div id="" class="book">
                                <div class="stt">${bookId++}. ${book[j]['name_book']}</div>
                                <div class="image"><img  style="width: 50px" src="${book[j]['image']}"></div>
                                <div class="category">Loại sách: ${book[j]['name_category']}</div>
                                <div class="amount-book">Số quyển: ${book[j]['amount']}</div>
                                <div class="total">Thành tiền: ${format(book[j]['money'])}</div>
                            </div>`;
            }
    ...
    for(let i = 0; i < listTotal.length; i++){
        for(let c = 0; c < cart.length; c++){
            if(cart[c]['total_money'] == listTotal[i]){
                ...
                var child =     `<div id="" class="info-user">
                                    <div class="name-user">${cartId}. Tên tài khoản: ${cart[c]['username']}</div>
                                    <div class="first-name">Họ tên: ${cart[c]['last_name']} ${cart[c]['first_name']}</div>
                                    <div class="number-phone">Số điện thoại: ${cart[c]['phone_number']}</div>
                                    <div class="total-money">Tổng tiền: ${format(cart[c]['total_money'])}</div>
                                 </div>`;
                ci = "c"+(cartId).toString();
                document.getElementById(ci).insertAdjacentHTML('afterbegin', child);
